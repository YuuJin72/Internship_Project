...
const TodaysWork = () => {
  ...
  const fetchPost = () => {
    ...
    axios
      .post(`http://localhost:8080/study/${params.id}/todoindiv`, {
        date: today,
      })
      .then((res) => {
  ...
  const handleSubmitTodo = () => {
    axios
      .post(`http://localhost:8080/study/${params.id}/todosubmit`, {
        date: today,
        title: todo,
      })
      .then((res) => {
  ...
  const handleDeleteTodo = (e) => {
    axios
      .post(`http://localhost:8080/study/${params.id}/tododelete`, {
        _id: e.target.id,
      })
      .then((res) => {
        if (res.data.message === "success") {
  ...
  const handleFinishTodo = () => {
    axios
      .post(`http://localhost:8080/study/${params.id}/todofinish`)
      .then((res) => {
        if (res.data.message === "success") {
          fetchPost();
