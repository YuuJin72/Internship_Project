...
const Settings = () => {
  ...
  const fetchPost = () => {
    axios
      .post(`http://localhost:8080/study/${params.id}/settingsr`)
      .then((res) => {
  ...
  const fetchMember = () => {
    axios
      .post(`http://localhost:8080/study/${params.id}/settingsm`)
      .then((res) => {
        if (res.data.message === "success") {
  ...
  const handleConfirm = (e) => {
    axios
      .post(`http://localhost:8080/study/${params.id}/memberconfirm`, {
        member: e.target.id,
      })
      .then((res) => {
        if (res.data.message === "success") {
  ...
  const handleNonConfirm = (e) => {
    axios
      .post(`http://localhost:8080/study/${params.id}/membernonconfirm`, {
        member: e.target.id,
      })
      .then((res) => {
        if (res.data.message === "success") {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(`http://localhost:8080/study/${params.id}/settingsave`, {
        ...
        >
          ...
          >
            <Grid container sx={{ p: 5 }}>
              <Grid item xs={12}>
                {/* <NoAuth/> */}
                <Typography variant="h6">방 제목 변경</Typography>
                <TextField
                  name="title"
                  value={post?.title || ""}
                  placeholder="방 제목 변경"
                ...
                <Typography sx={{ mt: 6, mb: 1 }} variant="h6">
                  멤버 관리
                </Typography>
              </Grid>
              <Grid item xs={6} textAlign="center" sx={{ mt: 1 }}>
                <Box
                  border={1}
                  borderRadius={5}
                  sx={{ p: 2, mr: 1, borderColor: "grey.400" }}
                >
                  신청 대기
                  {member &&
                    member.map(
              ...
              <Grid item xs={6} textAlign="center" sx={{ mt: 1 }}>
                <Box
                  border={1}
                  borderRadius={5}
                  sx={{ p: 2, ml: 1, borderColor: "grey.400" }}
                >
                  현재 멤버
                  {member &&
                    member.map(
                        ...
                        el.confirmed === 1 && (
                          ...
                          >
                            ...
                            <Grid item xs={6}>
                              ...
                              {el.confirmed === 1 && el.hostid !== el.id && (
                                <Button
                                  variant="contained"
                                  id={el.id}
                                  onClick={handleNonConfirm}
