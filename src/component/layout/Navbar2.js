...
const Navbar2 = () => {
  ...
  const handleLogout = () => {
    axios.post("http://localhost:8080/signout").then((res) => {
      handleMenuClose();
      Success("로그아웃 완료");
      ...
      <AppBar position="static" color="darkblue">
        <Toolbar>
          <img
            src={logo}
            alt="logo"
            style={{ height: "5rem", width: "8.5rem", cursor: "pointer" }}
            onClick={handleHome}
          />
          <Box sx={{ flexGrow: 1 }} />
          <Box
          ...
          >
            {user ? (
              <Typography sx={{ mr: 3 }}>{nickname} 님, 환영합니다!</Typography>
            ) : (
              <Typography sx={{ mr: 3 }}>
                로그인하여 서비스를 이용해보세요!
              </Typography>
            ...
            <IconButton size="large" color="inherit" onClick={handleStudy}>
              <AutoStoriesIcon />
            </IconButton>
            <IconButton
              size="large"
