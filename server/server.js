...
app.post("/signup", (req, res) => {
  ...
  con.query(sql, [req.body.email], (err, result) => {
    ...
    if (result.length === 0) {
      ...
      con.query(
        inssql,
        [req.body.id, req.body.password, req.body.email, req.body.nickname],
        (err, result) => {
          if (err) {
            res.json({ message: "fail" });
          } else {
    ...
    if (!req.isAuthenticated()) {
      ...
      con.query(sql, [Number(req.params.id)], (err, result) => {
        ...
        if (result.length === 0) {
          ...
          con.query(
            confirmSql,
            [Number(req.params.id), req.user[0].id],
            (err, result) => {
              if (err) throw err;
              if (result.length === 0) {
  ...
  con.query(
    ...
    (err, result) => {
      if (err) throw err;
        ...
        con.query(
          insSql,
          [result.insertId, req.user[0].id, 1],
          (err, result) => {
...
app.post("/study/requestmember/:id", (req, res) => {
  ...
  con.query(chkSql, [Number(req.params.id), req.user[0].id], (err, result) => {
    if (req.isAuthenticated() && result[0].total >= result[0].limit_member) {
      ...
      con.query(
        insSql,
        [Number(req.params.id), req.user[0].id, 0],
        (err, result) => {
          if (err) throw err;
...
app.post("/study/:id/todoindiv", (req, res) => {
  ...
  con.query(delSql, [Number(req.params.id), req.body.date], (err, result) => {
    if (err) throw err;
      ...
      con.query(
        sql,
        [Number(req.params.id), req.user[0].id, req.body.date],
        (err, result) => {
          if (err) res.json({ message: "err" });
          else
            res.json({
...
app.post("/study/:id/board/editend", (req, res) => {
  ...
  con.query(
    sql,
    [req.body.detail, Number(req.params.id), req.body.boardId],
    (err, result) => {
      if (err) throw err;
      else {
        res.json({
...
app.post("/myinfo/mystudy", (req, res) => {
  if (!req.isAuthenticated()) {
    ...
    con.query(
      sql,
      [req.user[0].id, req.user[0].id, req.user[0].id],
      (err, result) => {
        if (err) res.json({ message: "error" });
        else {
          res.json({
